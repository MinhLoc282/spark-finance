...
function DepositPopup({
  ...
  const submitDeposit = async () => {
    if (!amountInput) {
      ...
      const record = {
        ...
        from_subaccount: [],
        created_at_time: [],
        amount: BigInt(Big(amountInput).times(Big(10).pow(decimals))),
        expected_allowance: [],
        expires_at: [],
      ...
      try {
        ...
        const tx = await depositActor.deposit(
          ...
          BigInt(Big(amountInput).times(Big(10).pow(decimals))),
  ...
  const setMaxInput = () => {
    if (decimals && tokenBalance) {
      ...
      const integerPart = tokenBalance / divisor;
      ...
      const maxAmount = integerPart.toString() + (fractionalString ? `.${fractionalString}` : '');
  ...
  const handleInputChange = (e) => {
    ...
    if (value === '') {
      ...
      if (decimalParts.length === 1
        || (decimalParts.length === 2 && decimalParts[1].length <= decimals)) {
    ...
    >
        ...
        <div className={styles.LabelContainer}>
          <div className={styles.Label}>
            ...
            <div>
              Balance:
              {' '}
              {tokenBalance && decimals && <span>{Number(tokenBalance) / 10 ** decimals}</span>}
            </div>
          </div>
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px' }}>
        ...
        <div style={{ display: 'flex' }}>
          ...
          {amountInput && amountReceived(selectedOption) && <div style={{ marginLeft: '8px' }}>{Math.round(Number(amountInput) * (amountReceived(selectedOption)) * 1000) / 1000}</div>}
        </div>
      </div>
      <div style={{
