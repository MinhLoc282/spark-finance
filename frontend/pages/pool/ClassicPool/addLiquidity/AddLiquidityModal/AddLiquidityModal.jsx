...
function AddLiquidityModal({
  ...
  const formatTo9DecimalPlaces = (value) => {
    // Ensure value is parsed as Big.js number
    const bigValue = new Big(value || 0);
    // Round to 9 decimal places
    const roundedValue = bigValue.round(9, Big.roundDown);
    // Convert to string with fixed 9 decimal places
    const formattedValue = roundedValue.toFixed(9);
    ...
    >
      ...
      <div className={styles.TokenListContainer}>
        <div className={styles.LineContainer}>
          ...
          <div>
            <div>
              {formatTo9DecimalPlaces(formValues.amount0Desired)}
              {' '}
              {tokens[0]}
            </div>

            <div>
              {formatTo9DecimalPlaces(formValues.amount1Desired)}
              {' '}
              {tokens[1]}
            </div>
          </div>
