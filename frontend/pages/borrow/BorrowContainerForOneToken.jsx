...
function BorrowContainer({ pairName }) {
  ...
  useEffect(() => {
      ...
      if (principal) {
        try {
          ...
          if (token0CanisterId === token0.canisterId || token0CanisterId === token1.canisterId) {
            ...
            setBalanceToken0(parseFloat(tx2));
            setBalanceToken1(parseFloat(tx));
          }

          setBalanceLpToken(parseFloat(tx3));
          setBalanceDeposit(tx4[0].amount);

          setBorrowInfo(tx4[0]);
          setHealthRatio(Math.abs((Number(tx5) * 100)));
        } catch (error) {
          console.log(error);
        }
      }
    ...
    balanceToken();
  }, [principal, updateUI, borrowCanisterId, token0CanisterId, token1CanisterId]);
  ...
  useEffect(() => {
    const func = async () => {
      if (balanceLpToken) {
        const borrowActor = getActor(borrowCanisterId, identity);
        const tx1 = await
        borrowActor.getPairInfo(balanceLpToken + parseFloat(balanceDeposit));
  ...
  useEffect(() => {
    const getBorrowInfo = async () => {
      ...
      if (token0CanisterId === token0.canisterId
        || token0CanisterId === token1.canisterId
      ) {
        setAvaiBorrow([
          Math.min(
            parseFloat((tx[0] * BigInt(60)) / BigInt(100) - borrowInfo.borrow),
      ...
      } else if (token1CanisterId === token0.canisterId
    ...
    if (borrowCanisterId && borrowInfo) {
      getBorrowInfo();
    }
  }, [borrowCanisterId, balanceDeposit, balanceBorrowETH, balanceBorrowBTC, borrowInfo]);
  ...
  return (
    <div style={{ paddingTop: '70px' }}>
      ...
      <div className={styles.Container}>
        <div style={{
          textAlign: 'center', fontSize: '24px', marginBottom: '20px',
        }}
        >
          {pairName}
        </div>
        ...
        <div style={{
          textAlign: 'center', fontSize: '16px', marginBottom: '12px',
        }}
        >
          Deposit
          {' '}
          ...
          {token1Label}
          {' '}
          while earning trading fees
        </div>

        <div className={styles.Header}>
          <div className={styles.HeaderLeft}>
            <div className={styles.HeaderTitle}>TOTAL BORROWED</div>
            {(token0CanisterId !== deposit0.canisterId && token0CanisterId !== deposit1.canisterId)
            && (
            ...
            {(token1CanisterId !== deposit0.canisterId && token1CanisterId !== deposit1.canisterId)
            && (
            <div className={styles.Headerdiv}>
