...
function SupplyPopup({
  ...
  const submitDeposit = async () => {
    if (!amountInput) {
      ...
      try {
        ...
        if (tx.includes('Update') || tx.includes('Update')) {
  ...
  return (
    ...
    >
      ...
      <button type="button" className={styles.ButtonContainer} onClick={submitDeposit}>
        {loading ? 'Loading...' : 'Suppy'}
...
SupplyPopup.propTypes = {
  isSupplyModalOpen: PropTypes.bool.isRequired,
