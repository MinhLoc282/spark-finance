...
function WithdrawPopup({
  ...
  const changeAmountIn = (percentage) => {
    if (tokenBalance && !isActive) {
      const newAmountIn0 = Big(percentage)
        .times(Big(tokenBalance).div(Big(10).pow(18)))
        .div(Big(100));
  ...
  const handleMaxButtonClick = () => {
    if (!isActive) {
      const maxAmount = Big(tokenBalance).div(Big(10).pow(18));
      ...
      <div>
        <div className={styles.LabelContainer}>
          ...
          <div className={styles.RightLabel}>
            Balance:
            {' '}
            {!isActive ? Math.round((Number(tokenBalance) / 10 ** 18) * 1000) / 1000 : 0}
          </div>
        </div>
