...
shared (msg) actor class Deposit(
    ...
    private var rewardDepositInfo = Buffer.Buffer<DepositRewardType>(0);
    // private var rewardDepositInfo = HashMap.HashMap<Principal, [DepositRewardType]>(1, Principal.equal, Principal.hash);
    ...
    private var total_deposit : Nat = 0;
    private var total_multiplier : Nat = 0;
    ...
    private func getInterest(
        ...
        var X : Float = Float.fromInt(firstMultiplier);
        ...
        var result = (intetest / 100) * ((n + 1) * X - ((Float.fromInt(decayPerDay) * n * (n + 1)) / 2));

        if (lastUpdateTime > 0) {
            var Y : Float = Float.fromInt(lastUpdateTime) - 1;
            result := result - (intetest / 100) * ((Y +1) * X - ((Float.fromInt(decayPerDay) * Y * (Y +1)) / 2))
        };
        return result
    };

    private func compareTimestamps(t1 : Time.Time, t2 : Time.Time) : Int {
        let nanosecondsPerDay : Int64 = 86_400_000_000_000;
        let diffNanoseconds : Int64 = Int64.fromInt(t1 - t2);
        let diffDays : Int64 = diffNanoseconds / nanosecondsPerDay;
        return Int64.toInt(diffDays);
    };
    // Get mutiplier
    ...
    private func getTotalDckETH() : async Nat {
        ...
        for ((principal, deposits) in depositInfoCkETH.entries()) {
            ...
            for (deposit in deposits.vals()) {
                let depEle = deposit;
                let currentMul : Nat = await getCurrentMultiplier(depEle);
                if (deposit.isActive) {
                    total += currentMul;
                };
            };
            totalUsersDckETH += total;
        };

        Debug.print("Final total (as Nat)");
        Debug.print(debug_show(totalUsersDckETH));
        return totalUsersDckETH;
    };

    private func safeFloatToNat(f : Float) : Nat {
        if (f < 0 or f > Float.fromInt64(Int64.maximumValue)) {
            Debug.print("Warning: Float value out of safe Nat range");
            return 0;
        };
        let i = Int64.abs(Float.toInt64(Float.floor(f)));
        Nat64.toNat(Int64.toNat64(i))
    };
    ...
    private func getFirstMultiplier(value : Nat, period : Nat) : Nat {
        switch (period) {
            case (1) { return value * 10006 / 10000 };
            case (3) { return value * 10518 / 10000 };
            case (7) { return value * 11068 / 10000 };
            case (14) { return value * 11666 / 10000 };
            case (30) { return value * 12358 / 10000 };
            case (90) { return value * 13401 / 10000 };
            case (180) { return value * 14741 / 10000 };
            case (270) { return value * 16182 / 10000 };
            case (360) { return value * 17729 / 10000 };
            case (450) { return value * 19392 / 10000 };
            case (540) { return value * 21176 / 10000 };
            case (_) { return value * 10006 / 10000 }
        }
    };

    public func getCurrentMultiplier(userInfo : DepositType) : async Nat {
        let decayPerDay : Nat = await getDecayPerDay(userInfo);
        let daysHavePass : Int = compareTimestamps(Time.now(), userInfo.startTime);

        // Convert daysHavePass to Nat safely
        let daysNat : Nat = Nat64.toNat(Int64.toNat64(Int64.fromInt(daysHavePass)));
        return userInfo.firstMultiplier - (daysNat * decayPerDay);
    };

    public func getCurrentMultiplierBaseOnTimestamp(userInfo : DepositType, time : Time.Time) : async Nat {
        var decayPerDay : Nat = await getDecayPerDay(userInfo);
        var daysHavePass : Int = compareTimestamps(time, userInfo.startTime);
    ...
    private func getDecayPerDay(userInfo : DepositType) : async Nat {
        let firstMul : Nat = getFirstMultiplier(userInfo.amount, userInfo.duration);
        let deposit : Nat = userInfo.amount;
        let duration : Nat = userInfo.duration;

        // Ensure the divisor is not zero
        if (duration == 0) {
            Debug.print("Error: Duration cannot be zero");
            return 0;
        };

        return (firstMul - deposit) / duration;
    };
    ...
    public func getTotalInterestForUser(principal: Principal, days: Nat) : async Nat {
        var totalInterest : Nat = 0;
        let currentTime = Time.now();
        let timeThreshold = currentTime - days * 24 * 60 * 60 * 1_000_000_000;

        let userDeposits = depositInfoCkETH.get(principal);
        switch (userDeposits) {
            case (?deposits) {
                for (deposit in deposits.vals()) {
                    for (reward in rewardDepositInfo.vals()) {
                        if (reward.depositTime >= timeThreshold and reward.depositTime > deposit.lastClaimedTime and reward.depositTime > deposit.startTime) {
                            let currentMul = await getCurrentMultiplierBaseOnTimestamp(deposit, reward.depositTime);
                            let withdrawValue = (reward.amount * currentMul) / reward.totalShare;
                            totalInterest += withdrawValue;
                        };
                    };
                };
            };
    ...
    public shared func getShareOfInterest(userId : Principal) : async ?Float {
        let totalSupply = await getTotalInterestPaidOut(7);

        if (totalSupply > 0) {
            let totalInterest = await getTotalInterestForUser(userId, 7);
            Debug.print("Total of interest" #debug_show(totalInterest) #debug_show(totalSupply));
    ...
    public shared (msg) func getDepositStats() : async (
        shareOfInterest: ?Float,
        circulatingSupply: Nat,
        averageLock: ?Nat,
        globalAverageLockTime: ?Nat,
        balance: Nat,
        totalInterestPaidOut: Nat,
        totalSupply: Nat
    ) {
        let shareOfInterest = await getShareOfInterest(msg.caller);
        let circulatingSupply = await getCirculatingSupply();
        let averageLock = await getAverageLock(msg.caller);
        let globalAverageLockTime = await getGlobalAverageLockTime();
        var defaultSubaccount : Blob = Utils.defaultSubAccount();
        let balance = await icrc1_balance_of({
                        owner = msg.caller;
                        subaccount = ?defaultSubaccount
                      });
        let totalInterestPaidOut = await getTotalInterestPaidOut(7);
        let totalSupply = await icrc1_total_supply();
    ...
    public shared (msg) func getAccountInfo() : async (
        tokenBalance: Nat,
        wrapBalance: Nat,
        interestInfo: Nat,
        apy: Float
        ...
        let tokenBalance = await getTokenBalance(principal);
        let wrapBalance = await getWrapBalance(principal);
        let interestInfo = await getInterestInfo(principal);
        let apy = await calculateAPY(principal);
    ...
    public shared (msg) func depositReward(tokenId : Principal, value : Nat) : async TxReceipt {
        ...
        if (not tokens.hasToken(tid)) {
            return #err("Token does not exist");
        };

        let fee = tokens.getFee(tid);
        if (value <= fee) {
            return #err("Value must be greater than token transfer fee");
        };

        let tokenCanister = _getTokenActor(tid);

        let txid = switch (await _transferFrom(tokenCanister, msg.caller, value, fee)) {
            case (#Ok(id)) { id };
            case (#Err(e)) { return #err("Token transfer failed: " # tid) };
            case (#ICRCTransferError(e)) {
                Debug.print(debug_show(e));
                return #err("ICRC Transfer Error: " # debug_show(e));
            };
        };
        ...
        // Get total share
        let totalShare = await getTotalDckETH();
        if (totalShare == 0) {
            return #err("No deposit");
        };

        // Create new deposit information
        let newDepInform : DepositRewardType = {
            amount = value;
            depositTime = Time.now();
            totalShare = totalShare;
        };

        // Update rewardDepositInfo
        if (rewardDepositInfo.size() >= 1000) {
            ignore rewardDepositInfo.remove(0);
        };
        rewardDepositInfo.add(newDepInform);
    ...
    public shared func calculateAPY(userId: Principal) : async Float {
        var totalReward: Nat = 0;
        var totalUserLockAndUnlock : Nat = 0;
        let currentTime = Time.now();
        ...
        switch (maybeArray) {
            case (?r) {
                for (depEle in r.vals()) {
                    if (depEle.isActive) {
                      totalUserLockAndUnlock += depEle.amount;
                    }
                };
            };
            case (_) {}
        };
        if (totalUserLockAndUnlock == 0) {
            return 0.0;
        };

        let totalRewardFloat = Float.fromInt(totalReward);

        let apy = (totalRewardFloat / Float.fromInt(totalUserLockAndUnlock)) * 365;
        ...
        switch (maybeArray) {
            case (?r) {
                ...
                for (depEle in r.vals()) {
                    ...
                    var updateDay = compareTimestamps(t1, t2);
                    var currentMul : Nat = await getCurrentMultiplier(depEle);
    ...
    public shared func privateWithdraw(index : Nat, caller : Principal) : async ICRC1.TransferResult {
        ...
        switch (maybeArray) {
            case (?r) {
                ...
                var currentMul : Nat = await getCurrentMultiplier(r[index]);
    ...
    public shared (msg) func getInterestInfo(caller : Principal) : async Nat {
                    ...
                    if (depEle.isActive) {
                        ...
                        for (deposit in rewardDepositInfo.vals()) {
                            ...
                            if (deposit.depositTime > depEle.lastClaimedTime and deposit.depositTime > depEle.startTime) {
                                Debug.print("getCurrentMultiplier");
                                var currentMul : Nat = await getCurrentMultiplier(depEle);
                                // var dck = depEle.amount + currentInterest;
                                // var dck = Nat64.toNat(Int64.toNat64(Float.toInt64(currentMul)));
                                Debug.print("currentMul");
                                Debug.print(debug_show(currentMul));
                                var dck = currentMul;
                                var withdrawValue = (deposit.amount * dck) / deposit.totalShare;
                                Debug.print("withdrawValue");
                                Debug.print(debug_show(withdrawValue));
                                totalWithdraw += withdrawValue;
                                Debug.print("totalWithdraw");
                                Debug.print(debug_show(totalWithdraw));
                            };
                        };
                    }
                }
        ...
        Debug.print("final totalWithdraw");
        Debug.print(debug_show(totalWithdraw));
