...
shared (msg) actor class Borrow(
    ...
    public shared (msg) func borrow(
        ...
        switch (maybeArray) {
            case (?r) {
                ...
                if (r.tokenIdBorrow != tokenId_canister_borrow) {
                ...
                // if (Principal.toText(tokenId_canister_borrow) == principalToken0 and tokenId_canister_borrow == token0) {
                //     if (borrowValue > amountToken0 * 60 / 100) {
                //         return "Exceed 60% of acceptable borrowing"
                //     }
                // } else if (Principal.toText(tokenId_canister_borrow) == principalToken1 and tokenId_canister_borrow == token0) {
    ...
    public shared (msg) func sendTokenToLendingCanister(tokenId: Principal, amount: Nat) : async ICRC1.TransferResult {
      Debug.print(debug_show(deposit_id));
      Debug.print(debug_show(msg.caller));
      if (msg.caller != deposit_id) {
    ...
    public shared func sendInterestToLendingCanister() : async Text {
        for ((principal, deposit) in depositInfoLpToken.entries()) {
            ...
            var deposit_canister = actor (Principal.toText(deposit_id)) : actor {
