...
shared (msg) actor class Borrow(
    ...
    type DepositType = {
        amount : Nat;
        interest : HashMap.HashMap<Principal, Nat>;
        startTime : Time.Time;
        duration : Nat;
        isActive : Bool;
            ...
            case (#Ok _) {
                ...
                switch (maybeArray) {
                    ...
                    case (_) {
                        var newDepInform : DepositType = {
                            amount = lpValue;
                            interest = HashMap.HashMap<Principal, Nat>(0, Principal.equal, Principal.hash);
                            startTime = 0;
                            duration = 0;
                            isActive = true;
    ...
    public shared (msg) func borrow(
        ...
        switch (maybeArray) {
            case (?r) {
                ...
                if (r.tokenIdBorrow != tokenId_canister_borrow and r.borrow > 0) {
                    return "You have an existing borrow for a different token. Please repay first."
                };
                ...
                var currentBorrowed = r.borrow;
                var maxBorrowable : Nat = 0;

                if (Principal.toText(tokenId_canister_borrow) == principalToken0) {
                    maxBorrowable := amountToken0 * 60 / 100;
                } else if (Principal.toText(tokenId_canister_borrow) == principalToken1) {
                    maxBorrowable := amountToken1 * 60 / 100;
                } else {
                    return "Invalid Token Canister"
                };

                if (currentBorrowed + borrowValue > maxBorrowable) {
                    ...
                    case (#Ok _) {
                        ...
                        switch (maybeArray) {
                            case (?r) {
                                ...
                                let borrowedTokenInterest = switch (newDepInform.interest.get(r.tokenIdBorrow)) {
                                    case (?interest) {
                                ...
                                newDepInform.interest.put(r.tokenIdBorrow, borrowedTokenInterest);
    ...
    public shared func sendInterestToLendingCanister() : async Text {
        for ((principal, deposit) in depositInfoLpToken.entries()) {
            for ((tokenId, interest) in deposit.interest.entries()) {
                ...
                if (interest > 1000000000000) {
                    let approve : ICRC1.ApproveResult = await token_canister.icrc2_approve {
                        from_subaccount = null;
                        spender = if (Principal.toText(tokenId) == Principal.toText(token0)) {
                            deposit0_id;
                        } else {
                            deposit1_id;
                        };
                    ...
                    var txResult = await deposit_canister.depositReward(tokenId, interest);

                    // Reset the interest for the specific token
                    deposit.interest.put(tokenId, 0);
                }
            };

            depositInfoLpToken.put(principal, deposit);
        };
            ...
            case (#Ok _) {
                ...
                switch (maybeArray) {
                    case (?r) {
                        ...
                        let borrowedTokenInterest = switch (newDepInform.interest.get(tokenId)) {
                            case (?i) {
                                i + interest;
                            };
                            case (null) {
                        ...
                        newDepInform.interest.put(tokenId, borrowedTokenInterest);
    ...
    public shared (msg) func getDepositId() : async ?SharedDepositType {
        switch (depositInfoLpToken.get(msg.caller)) {
            case (?deposit) {
                let sharedDeposit : SharedDepositType = {
                    amount = deposit.amount;
                    startTime = deposit.startTime;
                    duration = deposit.duration;
                    isActive = deposit.isActive;
                    tokenIdBorrow = Principal.toText(deposit.tokenIdBorrow);
    ...
    public func getDepositIdPerUser(user : Principal) : async ?SharedDepositType {
        switch (depositInfoLpToken.get(user)) {
            case (?deposit) {
                let sharedDeposit : SharedDepositType = {
                    amount = deposit.amount;
                    startTime = deposit.startTime;
    ...
    private var remove_rate : Float = 0.8;
