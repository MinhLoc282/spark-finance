...
shared (msg) actor class Swap(owner_ : Principal, swap_id : Principal) = this {
    ...
    // private stable var tokenFee : Nat = 0; // 0.0001 if decimal == 8
    ...
    // private stable let minimum_liquidity : Nat = 0;
    ...
    */
    ...
    public shared (msg) func addToken(tokenId : Principal, tokenType : Text) : async TxReceipt {
        ...
        ignore addRecord(
            msg.caller,
            "addToken",
            [("tokenId", #Text(Principal.toText(tokenId)))],
        );
        txcounter += 1;
    ...
    public shared (msg) func deposit(tokenId : Principal, value : Nat) : async TxReceipt {
        ...
        let tokenCanister = _getTokenActor(tid);
        // return #err(Principal.toText(msg.caller) # " " # Principal.toText(Principal.fromActor(this)));
        // return #err("ERR:" # tid);
        ...
        let txid = switch (result) {
            ...
            case(#ICRCTransferError(e)) {
                switch (e) {
                    ...
                    case (#CustomError(text)) {return #err("CustomError: " # text )};
                    ...
                    case (#InsufficientFunds) { return #err("InsufficientFunds") };
                    case (#TemporarilyUnavailable) { return #err("TemporarilyUnavailable") };
    ...
    private func depositForUser(userPId : Principal, tokenId : Principal) : async TxReceipt {
        ...
        if (balance < tokens.getFee(tid)) return #err("value less than token transfer fee");
                ...
                switch (await _transfer(tokenCanister, msg.caller, value - fee)) {
                    ...
                    case (#Err(e)) {
                        ...
                        var ledger_error = switch (e) {
                            case (#Other(errMsg)) {
                                if (
                                    tid == "utozz-siaaa-aaaam-qaaxq-cai" or tid == "lzvjb-wyaaa-aaaam-qarua-cai" or tid == "djjfs-tyaaa-aaaak-qagtq-cai"
                                ) {
                                    true
                                } else {
                                    false
    ...
    */
    public shared (msg) func createPair(token0 : Principal, token1 : Principal) : async TxReceipt {
        ...
        if (tid0 == tid1 or token0 == blackhole or token1 == blackhole) return #err("identical addresses or blackhole address");
        if (tokens.hasToken(tid0) == false or tokens.hasToken(tid1) == false) return #err("token not exist");

        let (t0, t1) = Utils.sortTokens(tid0, tid1);
        let pair_str = t0 # ":" # t1;
        if (Option.isSome(pairs.get(pair_str)) or lptokens.hasToken(pair_str)) return #err("pair exists");
    ...
    */
    ...
    //     if (amount0Desired == 0 or amount1Desired == 0) return #err("desired amount should not be zero");
    ...
    //     };
    //     if (tokens.zeroFeeTransfer(pair.token0, msg.caller, Principal.fromActor(this), amount0) == false) return #err("insufficient balance: " # pair.token0);
    //     if (tokens.zeroFeeTransfer(pair.token1, msg.caller, Principal.fromActor(this), amount1) == false) return #err("insufficient balance: " # pair.token1);
    ...
    //     assert (lpAmount > 0); //// assert fail right here!
    //     // return #err("Reach this checkpoint !");
    ...
    */
    public shared (msg) func addLiquidityForUser(
        ...
        if (tokens.zeroFeeTransfer(pair.token0, userPId, Principal.fromActor(this), amount0) == false) return #err("insufficient balance: " # pair.token0);
        if (tokens.zeroFeeTransfer(pair.token1, userPId, Principal.fromActor(this), amount1) == false) return #err("insufficient balance: " # pair.token1);
    ...
    public shared (msg) func validateRegisterFundRecoveryForUser(user : Principal, tokenId : Principal, amount : Nat) : async ValidateFunctionReturnType {
        let tid : Text = Principal.toText(tokenId);
        if (tokens.hasToken(tid) == false) return #Err("token does not exists");
        switch (userFundRecoveries.get(user)) {
            case (?r) {
                return #Err("pending funding recovery")
            };
    ...
    public shared (msg) func addLiquidityForUserTest(
        ...
        if (amount0Desired == 0 or amount1Desired == 0) return "desired amount should not be zero";
    ...
    */
    public shared (msg) func removeLiquidity(
        ...
        if (amount0 == 0 or amount1 == 0) return #err("insufficient LP tokens");
        ...
        if (Nat.greater(feeLP, 0)) {
            ...
            if (amount0 < amount0calculated or amount1 < amount1calculated) return #err("insufficient output amount : " # debug_show ({ amount0 = amount0; amount1 = amount1; amount0Part0 = amount0Part0; amount0Part1 = amount0Part1; amount1Part0 = amount1Part0; amount1Part1 = amount1Part1; amount0calculated = amount0calculated; amount1calculated = amount1calculated }))
        } else {
    ...
    public query func getTotalAPR(token0 : Principal, token1 : Principal): async Float {
        let pair = switch (_getPair(Principal.toText(token0), Principal.toText(token1))) {
            case (?p) { p };
            case (_) {
                return 0.0;
        ...
        var totalETHSwapped = 0;
        var totalBTCSwapped = 0;

        for ((_, swapTx) in swapTransactions.entries()) {
            if (swapTx.token == pair.token0) {
                totalETHSwapped += swapTx.amount;
            } else if (swapTx.token == pair.token1) {
                totalBTCSwapped += swapTx.amount;
            };
        };

        let aprETH : Float = 0.002 * Float.fromInt(totalETHSwapped) / Float.fromInt(ethLiquidity) * 365.0;
        let aprBTC : Float = 0.002 * Float.fromInt(totalBTCSwapped) / Float.fromInt(btcLiquidity) * 365.0;
    ...
    public shared (msg) func swapExactTokensForTokens(
        ...
        if (tokens.zeroFeeTransfer(path[0], msg.caller, Principal.fromActor(this), amounts[0]) == false) return #err("insufficient balance: " # path[0]);
    ...
    */
    ...
    system func inspect({
        ...
        msg : {
            ...
            #getTotalAPR: () -> (Principal, Principal);
        ...
        switch (msg) {
            ...
            case (#addLiquidity d) {
                ...
                if (amount0Desired == 0 or amount1Desired == 0) return false;
